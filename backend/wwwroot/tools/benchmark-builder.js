...
var computeAverage = ({ data }) => {
  const validData = data.filter(import_lodash.default.isNumber);
  return validData.length ? import_lodash.default.mean(validData) : 0;
};
var computeVariance = ({ data }) => {
  const validData = data.filter(import_lodash.default.isNumber);
  if (validData.length <= 1) return 0;
  const mean = computeAverage({ data: validData });
  return import_lodash.default.sum(validData.map((x) => (x - mean) ** 2)) / (validData.length - 1);
};
var round = (v) => Math.round(v * 1e3) / 1e3;
var arrayToMarkdownTable = ({ data, headerMap }) => {
  if (!data.length) return "";
  const headers = Object.values(headerMap);
  const rows = data.map(
    (row) => Object.keys(headerMap).map((key) => row[key] || "").join(" | ")
  );
...
function run(input) {
  const { suites, unit } = input;
  if (!suites || !Array.isArray(suites) || suites.length === 0) {
    return { error: "At least one suite is required" };
  }
  const results = suites.map(({ data: dirtyData, title }) => {
    const data = dirtyData.filter(import_lodash.default.isNumber);
    return {
      title,
      size: data.length,
  ...
  }).sort((a, b) => a.mean - b.mean).map(({ mean, variance, size, title }, index, sortedSuites) => {
    const cleanUnit = unit ? unit.trim() : "";
    const bestMean = sortedSuites[0].mean;
    ...
    return {
      ...
      variance: `${round(variance)}${cleanUnit}${cleanUnit ? "\xB2" : ""}`,
      size
    };
  });
  const headers = {
  ...
  return {
    results,
    headers,
    markdown: arrayToMarkdownTable({ data: results, headerMap: headers }),
    bulletList: results.flatMap(({ title, ...sections }) => [
      ` - ${title}`,
      ...Object.entries(sections).map(
        ([key, value]) => `    - ${headers[key] ?? key}: ${value}`
      )
    ]).join("\n")
  };
}
